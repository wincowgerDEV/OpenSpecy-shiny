...
#       fill <- as_OpenSpecy(as.numeric(unique(libraryR()$all_variables)), spectra = data.frame(rn))
#       match_spec(data,
#                  library = libraryR(),
#                  na.rm = T,
#                  fill = fill)
...
#         }
#         else if (input$id_strategy == "model") {
...
#     #req(input$active_identification)
#     #req(!grepl("^ai$", input$id_strategy))
#     if (isTruthy(!grepl("^model$", input$lib_type))) {
#       if (!input$active_identification) {
...
#             lengthChange = FALSE,
#             pageLength = 5
#           ), rownames = FALSE, filter = "top", caption = "Selectable Matches", style = "bootstrap", class = "table table-dark", selection = list(mode = "single", selected = c(1)))
#     } else{
#       NULL
#     }
#   })
...
#                   NULL
#               )
#             ), fluidRow(column(6, plotOutput("particle_plot", height = "25vh")), column(6, plotOutput("material_plot", height = "25vh"))))
#       )
#     }
#     else{
#       NULL
...
#     req(!is.null(preprocessed$data))
#     req(names(max_cor()))
#     if(isTruthy(!is.null(preprocessed$data) && names(max_cor()))){
# 
...
#       ) %>%
#         popover(
#           title = "Options for downloading spectra and metadata from the analysis. Test Data is a Raman HDPE spectrum in csv format. Test Map is an FTIR ENVI file of a CA particle. Your Spectra will download your data with whatever processing options are active. Library Spectra will download the current library selected. Top Matches downloads the top identifications in the active analysis. All Matches will download all identifications with the library. Thresholded Particles will download a version of your spectra using the active thresholds selected to infer where particles are in spectral maps, particle spectra are collapsed to their medians and locations to their centroids.",
...
#     if(isTruthy(ncol(preprocessed$data$spectra) >= 1 &&
#                 input$active_identification &&
#                 input$download_selection == "Top Matches" &&
#                 !grepl("^model$", input$lib_type)
#                 )){
#     tagList(
#       numericInput(
